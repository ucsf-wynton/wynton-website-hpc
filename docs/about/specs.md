...
The job scheduler is SGE 8.1.9 ([Son of Grid Engine]) which provides [queues]({{ site.baseurl }}/scheduler/queues.html) for both communal and lab-priority tasks.
...
The [cluster can be accessed]({{ site.baseurl }}/get-started/access-cluster.html) via SSH to one of two login nodes:
...
The cluster has development nodes for the purpose of validating scripts, prototyping pipelines, compiling software, and more.  Development nodes [can be accessed from the login nodes]({{ site.baseurl }}/get-started/development-prototyping.html).
...
The compute nodes can only be utilized by [submitting jobs via the scheduler]({{ site.baseurl }}/scheduler/submit-jobs.html) - it is _not_ possible to explicitly log in to compute nodes.
...
Each user may use up to 500 GiB disk space in the home directory.  It is _not_ possible to expand user's home directory.  Research groups can add additional storage space under `/wynton/group` by either mounting their existing storage or [purchase new]({{ site.baseurl }}/about/pricing-storage.html).
...
[host_table.tsv]: {{ site.baseurl }}/assets/data/host_table.tsv
