...
In all other cases, you will be prompted to authenticate through a Wynton-specific 2FA method when SSH:ing directly to the cluster.  See [Two-Factor Authentication for SSH]({{ site.baseurl }}/hpc/get-started/duo-signup.html) for instructions.
</div>
...
To access the {{ site.cluster.name }} cluster you need an account - any UCSF affiliate can get an [account]({{ site.baseurl }}/hpc/about/join.html).  Anyone with a {{ site.cluster.name }} account, can access the cluster via _Secure Shell (SSH)_.
...
[development node]: {{ site.baseurl }}/hpc/get-started/development-prototyping.html
[X2Go]: {{ site.baseurl }}/hpc/howto/gui-x11fwd.html
