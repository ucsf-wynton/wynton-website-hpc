...
In this example we will run compute jobs that output the name of the compute node that runs the job, waits ten seconds to emulate some processing, and the time it runs.  The name of the current machine is available in environment variable `HOSTNAME` (standard in Unix) and the current time is outputted when calling the command `date`.  To do this as a compute job, create a script `~/tests/hello_world` containing:
...
_Hint_: To create this file, make sure that the folder exists first.  If it doesn't, call `mkdir ~/tests`.
...
This, in combination with the so called "she-bang" (`#! ...`) on the first line, allows you to call the script just any other software, e.g.
...
When submitting a job, the scheduler assigns the job an identifier ("job id").  In the above example, the job id is '201'.  Immediately after the job has been submitted, we can see that it is queued but not launched;
...
Later, when the job has been launched on one of the compute nodes, and we will see something like:
...
Eventually, when the job script finishes, `qstat` will no longer list it (if you have no other jobs on the queue, `qstat` will not output anything).
