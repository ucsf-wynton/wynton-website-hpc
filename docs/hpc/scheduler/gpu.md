...
If you see an error similar to `RuntimeError: CUDA error: no kernel image is available for execution on the device` this means your software has been compiled against a CUDA version which is not compatible with the GPU in the node your job is trying to run on. CUDA versions are usually backwards compatible. However, newer GPUs, like the A40 GPUs in the `qb3-atgpu` nodes, require a minimum CUDA version of cuda-11.1. If your software is compiled against an earlier version of CUDA it will not run on the `qb3-atgpu` GPU nodes. To fix, recompile your software against the Wynton `cuda/11.0` module (preferable) or avoid the `qb3-atgpu` nodes (not recommended, as these are the fastest and most numerous institutional GPUs in the Wynton cluster. To tell the scheduler to skip the `qb3-atgpu` nodes, use limitation `-l h='!qb3-atgpu*` on the command line or in your job script.
