...
* **Specify how much local scratch (TMPDIR) storage your job will need**.  Local storage is limited to [{{ site.data.specs.local_scratch_size_min }}-{{ site.data.specs.local_scratch_size_max }} TiB/node]({{ site.baseurl }}/hpc/about/specs.html).  If your job will use up to 200 GiB of disk space, you can specify this resource as `-l scratch=200G` (in units of GiB) when submitting the job.  A node with 800 GiB of scratch space can support up to four `-l scratch=200G` jobs running at the same time.
...
To understand the purpose of `qstat -j` at the end, see the [Job Summary]({{ site.baseurl }}/hpc/scheduler/job-summary.html) page.
