...
_Comment_: There are no user or group quotas on `/wynton/scratch/`, but [files on `/wynton/scratch/` that are older than two weeks are deleted automatically]({{ site.baseurl }}/about/specs.html).
